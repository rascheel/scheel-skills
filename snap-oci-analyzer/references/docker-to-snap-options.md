...
│   │                                  (exports ALL container env vars, not just PATH;
...
> If the entrypoint itself is a script with a `#!/usr/bin/env <interp>` (or any
> `#!...`) shebang, `create_wrapper.sh` prepends the resolved interpreter path
> explicitly, e.g. `... LD_LIBRARY_PATH="..." $SNAP/usr/bin/bash "$SNAP/entrypoint"
> $APP_ARGS "$@"` — this bypasses the kernel's own shebang parsing, which would
> otherwise invoke the BASE SNAP's `/usr/bin/env` and risk a glibc-mismatch crash
> if it inherits `LD_LIBRARY_PATH` (see `system-usernames-guide.md` §8).
